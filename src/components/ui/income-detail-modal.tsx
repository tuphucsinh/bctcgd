...
import { getIncomeTransactions } from "@/lib/actions";
...
export function IncomeDetailModal({ children, currentUser }: IncomeDetailModalProps) {
  ...
  const fetchData = useCallback(async (id: string) => {
    setLoading(true);
    try {
      const data = await getIncomeTransactions(id);
      setTransactions(data as Transaction[] || []);
    } catch (err) {
      console.error("Lỗi khi tải chi tiết thu nhập:", err);
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <div className="px-4 pt-8 pb-4 flex-1 overflow-y-auto custom-scrollbar">
          {loading ? (
            ...
            <div className="py-20 text-center space-y-2 opacity-40 italic">
              <p className="text-sm">Chưa có giao dịch thu nhập nào.</p>
            </div>
          ) : (
                ...
                {transactions.map((trx, idx) => (
                  ...
                  >
                    <div className="flex flex-col items-center justify-center min-w-[28px] h-full border-r border-white/5 pr-2.5">
                       <span className="text-sm font-black text-white/40 group-hover:text-emerald-500/60 transition-colors">
